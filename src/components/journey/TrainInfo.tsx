...
export default function TrainInfo({
  ...
  const constructName = (train: JourneyT) =>
    `${train.line?.productName}(${train.line?.fahrtNr})`;
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 border rounded-lg shadow-md">
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2 font-semibold">
          ...
          <span>{train.line?.operator?.name}</span>
        </div>
        <div className="flex items-center gap-2">
          <TrainFront className="w-5 h-5 text-gray-600" />
          <span>{train.line?.fahrtNr}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <span>{train.line?.productName}</span>
        </div>
        <div className="flex items-center gap-2">
          <Route className="w-5 h-5 text-gray-600" />
          <span>
            {train.stopovers ? train.stopovers[0].stop?.name : ""} →{" "}
            {train.stopovers
      ...
      {/* Canceled Stops */}
      {train.stopovers?.some((s) => s.cancelled) && (
      ...
      {wingId && (
        <div className="border-t col-span-1 sm:col-span-2 pt-3 mt-3 w-full min-w-0 whitespace-normal break-words">
          Fährt von {wingStart} bis {wingDest} vereeint mit {wingName}
          {/*<a
