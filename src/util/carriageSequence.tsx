...
const amenityIcons = new Map<string, { icon: JSX.Element; label: string }>([
  ...
  ["SEATS_SEVERELY_DISABLED", { icon: <Accessibility size={16} />, label: "Behindertenplätze" }],
  ["CABIN_INFANT", { icon: <Baby size={16} />, label: "Kleinkindabteil" }],
  ["WHEELCHAIR_SPACE", { icon: <Accessibility size={16} />, label: "Rollstuhlplatz" }],
  ["TOILET_WHEELCHAIR", { icon: <Accessibility size={16} />, label: "Barrierefreies WC" }],
...
export function parseAmenities(amenityType: string) {
  ...
  return amenityIcon ? (
    <Tooltip content={amenityIcon.label}>
      {amenityIcon.icon}
    </Tooltip>
  ) : null;
}
