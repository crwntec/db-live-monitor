...
const amenityIcons = {
  ...
  SEATS_BAHN_COMFORT: { icon: <Gem size={16} />, label: "Komfort-Sitzplätze" },
...
export function parseUICIdentifier(identifier) {
...
export function parseCountrycode(code) {
  const UICCountryCodes = {
  ...
  return UICCountryCodes[code] || { long: "Unbekannt", short: "XX" };
}

export function parseClassNumber(classNumber) {
  return `BR ${classNumber.substring(1)}`;
}

export function formatUIC(identifier) {
  const { exchangeCode, countryCode, classNumber, vehicleNumber } =
    parseUICIdentifier(identifier);
  return `${exchangeCode}-${countryCode}-${classNumber}-${vehicleNumber}`;
}

export function getLongNameForClassNumber(classNumber) {
  ...
  switch (_classNumber) {
    ...
    case 193:
      ...
      case 812:
        if (classNumber.substring(0,1)=='0'||classNumber.substring(0,1)=='5')return "412.0 (ICE 4 12tlg.)";
...
export function parseAmenities(amenityType) {
  return amenityIcons[amenityType] ? (
    <Tooltip content={amenityIcons[amenityType].label}>
      {amenityIcons[amenityType].icon}
    </Tooltip>
  ) : null;
}
